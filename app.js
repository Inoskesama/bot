console.log('')
console.log('-------------------------------')
console.log('  Скрипт Bot запущен.')
console.log('-------------------------------')
console.log('')
...
const farms = [
	...
	{		
		name: 'ASUS GeForce RTX 2080',
		cost: 63250,
		id: 2
	},
	{	
		name: 'MSI GeForce RTX 2080 Ti',
		cost: 96990,
		id: 3
	},
	{
...
${message.user.misc.farm === 1 ? '🔹' : '🔸'} 1. GIGABYTE GeForce RTX 2070 3₿/час (43810 руб)
${message.user.misc.farm === 2 ? '🔹' : '🔸'} 2. GSUS GeForce RTX 2080 9₿/час (63250 руб)
${message.user.misc.farm === 3 ? '🔹' : '🔸'} 3. MSI GeForce RTX 2080 Ti 81₿/час (96990 руб)
${message.user.misc.farm === 4 ? '🔹' : '🔸'} 4. NVIDIA Titan RTX 150₿/час (222000 руб)
...
Для покупки введите "Фермы [номер] [кол-во]"`);
	...
	else if(message.user.balance >= sell.cost)
	{
		...
		message.args[2] = message.args[2].replace(/(м|m)/ig, '000000');
			...
			saveUsers();
			return bot(`вы купили ${sell.name} (x${message.args[2]}) за ${utils.sp(sell.cost)}Руб
💰 Ваш баланс: ${utils.sp(message.user.balance)}Руб`);
		...
		saveUsers();
		return bot(`вы купили ${sell.name} (x1) за ${utils.sp(sell.cost)}Руб
💰 Ваш баланс: ${utils.sp(message.user.balance)}Руб`);
		}	
	}
});
...
Для покупки введите "Фермы [номер] [кол-во]"`);
	...
	if(message.user.balance < 43810) return bot(`недостаточно денег ${smileerror}`);
	else if(message.user.balance >= 43810)
	{

		var pricefarms1 = 43810*message.args[1];
		...
		saveUsers();
		return bot(`Вы купили GIGABYTE GeForce RTX 2070 (x${Number(message.args[1])}) за ${utils.sp(pricefarms1)}$
💰 Ваш баланс: ${utils.sp(message.user.balance)}$`);
...
Для покупки введите "Фермы [номер] [кол-во]"`);
	...
	if(message.user.balance < 63250) return bot(`недостаточно денег ${smileerror}`);
	else if(message.user.balance >= 63250)
	{

		var pricefarms3 = 63250*message.args[1];
		...
		saveUsers();
		return bot(`GSUS GeForce RTX 2080  (x${Number(message.args[1])}) за ${utils.sp(pricefarms3)}$
💰 Ваш баланс: ${utils.sp(message.user.balance)}$`);
...
cmd.hear(/^(?:бонус|🔑 Бонус|@destinybot 🔑 Бонус)$/i, async (message, bot) => {
	...
	if (prize === 2) {
	    message.user.balance += 50000;
		return bot(`вы выиграли 5000.000$ ${smilesuccess}`);
...
cmd.hear(/^(?:ферма|🔋 Ферма|@SpokersBot 🔋 Ферма)$/i, async (message, bot) => {
